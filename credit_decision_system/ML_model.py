...
pipelines = {
    "log_reg": Pipeline(
        [("prep", preprocessor_scaled), ("model", LogisticRegression(max_iter=1000))]
    ),
    "rf": Pipeline(
        [
...
for name, info in results.items():
    ...
    print(
        "Combined AUC (using combined score as ranking):",
        f"{roc_auc_score(y_test, combined_score):.3f}",
    )
    print("Model-only Classification Report:")
    print(
        classification_report(y_test, (model_proba >= 0.5).astype(int), zero_division=0)
    )
